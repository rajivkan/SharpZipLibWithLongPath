	...
	public class TarArchive : IDisposable
	{
		...
		void ExtractEntry(string destDir, TarEntry entry)
		{
			...
			if (Alphaleonis.Win32.Filesystem.Path.IsPathRooted(name)) {
				...
				name = name.Substring(Alphaleonis.Win32.Filesystem.Path.GetPathRoot(name).Length);
			}
			
			name = name.Replace('/', Alphaleonis.Win32.Filesystem.Path.DirectorySeparatorChar);
			
			string destFile = Alphaleonis.Win32.Filesystem.Path.Combine(destDir, name);
			
			if (entry.IsDirectory) {
				EnsureDirectoryExists(destFile);
			} else {
				string parentDirectory = Alphaleonis.Win32.Filesystem.Path.GetDirectoryName(destFile);
				...
				if (process) {
					...
					Stream outputStream = Alphaleonis.Win32.Filesystem.File.Create(destFile);
		...
		void WriteEntryCore(TarEntry sourceEntry, bool recurse)
		{
			...
			if (asciiTranslate && !entry.IsDirectory) {
				...
				if (!IsBinary(entryFilename)) {
					tempFileName = Alphaleonis.Win32.Filesystem.Path.GetTempFileName();
					
					using (StreamReader inStream  = Alphaleonis.Win32.Filesystem.File.OpenText(entryFilename)) {
						using (Stream outStream = Alphaleonis.Win32.Filesystem.File.Create(tempFileName)) {
			...
			if (entry.IsDirectory) {
				...
				if ( (tempFileName != null) && (tempFileName.Length > 0) ) {
					Alphaleonis.Win32.Filesystem.File.Delete(tempFileName);
				}
		...
		{
			if (!Alphaleonis.Win32.Filesystem.Directory.Exists(directoryName)) {
				try {
					Alphaleonis.Win32.Filesystem.Directory.CreateDirectory(directoryName);
				}
				catch (Exception e) {
		...
		{
			using (FileStream fs = Alphaleonis.Win32.Filesystem.File.OpenRead(filename))
			{
