...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class FastZip
	{
		...
		{
			CreateZip(Alphaleonis.Win32.Filesystem.File.Create(zipFileName), sourceDirectory, recurse, fileFilter, directoryFilter);
		}
		...
		{
			CreateZip(Alphaleonis.Win32.Filesystem.File.Create(zipFileName), sourceDirectory, recurse, fileFilter, null);
		}
		...
		{
            Stream inputStream = Alphaleonis.Win32.Filesystem.File.Open(zipFileName, FileMode.Open, FileAccess.Read, FileShare.Read);
					   ...
					   bool isStreamOwner)
		{
			...
			using (zipFile_ = new ZipFile(inputStream)) {
				...
				while (continueRunning_ && enumerator.MoveNext()) {
					...
					if (entry.IsFile)
					{
						// TODO Alphaleonis.Win32.Filesystem.Path.GetDirectory can fail here on invalid characters.
						if (directoryFilter_.IsMatch(Alphaleonis.Win32.Filesystem.Path.GetDirectoryName(entry.Name)) && fileFilter_.IsMatch(entry.Name)) {
							ExtractEntry(entry);
						}
					}
		...
		void ProcessFile(object sender, ScanEventArgs e)
		{
			...
			if ( e.ContinueRunning ) {
                try {
                    ...
                    using (FileStream stream = Alphaleonis.Win32.Filesystem.File.Open(e.Name, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
		...
		void ExtractFileEntry(ZipEntry entry, string targetName)
		{
			...
			if ( overwrite_ != Overwrite.Always ) {
				if ( Alphaleonis.Win32.Filesystem.File.Exists(targetName) ) {
			...
			if ( proceed ) {
				...
				if ( continueRunning_ ) {
					try {
						using ( FileStream outputStream = Alphaleonis.Win32.Filesystem.File.Create(targetName) ) {
							if ( buffer_ == null ) {
								buffer_ = new byte[4096];
							}
							if ((events_ != null) && (events_.Progress != null))
						...
						if ( restoreDateTimeOnExtract_ ) {
							Alphaleonis.Win32.Filesystem.File.SetLastWriteTime(targetName, entry.DateTime);
						}
						...
						if ( RestoreAttributesOnExtract && entry.IsDOSEntry && (entry.ExternalFileAttributes != -1)) {
							...
							Alphaleonis.Win32.Filesystem.File.SetAttributes(targetName, fileAttributes);
						}
#endif						
					}
					catch(Exception ex) {
		...
		void ExtractEntry(ZipEntry entry)
		{
			...
			if ( doExtraction ) {
					if ( entry.IsDirectory ) {
						...
						dirName = Alphaleonis.Win32.Filesystem.Path.GetDirectoryName(Alphaleonis.Win32.Filesystem.Path.GetFullPath(targetName));
					}
			}
			
			if ( doExtraction && !Alphaleonis.Win32.Filesystem.Directory.Exists(dirName) ) {
				if ( !entry.IsDirectory || CreateEmptyDirectories ) {
					try {
						Alphaleonis.Win32.Filesystem.Directory.CreateDirectory(dirName);
					}
					catch (Exception ex) {
						doExtraction = false;
						if ( events_ != null ) {
		...
		static bool NameIsValid(string name)
		{
			return (name != null) &&
				(name.Length > 0) &&
				(name.IndexOfAny(Alphaleonis.Win32.Filesystem.Path.InvalidPathChars) < 0);
		}
#else
		static bool NameIsValid(string name)
		{
			return (name != null) &&
				(name.Length > 0) &&
				(name.IndexOfAny(Alphaleonis.Win32.Filesystem.Path.GetInvalidPathChars()) < 0);
		}
#endif
		#endregion
