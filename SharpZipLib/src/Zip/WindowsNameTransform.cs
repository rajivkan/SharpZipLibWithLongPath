	...
	public class WindowsNameTransform : INameTransform
	{
		...
		public string BaseDirectory
		{
			...
			set {
				...
				_baseDirectory = Alphaleonis.Win32.Filesystem.Path.GetFullPath(value);
			}
		}
		...
		public string TransformFile(string name)
		{
			if (name != null) {
				...
				if ( _trimIncomingPaths ) {
					name = Alphaleonis.Win32.Filesystem.Path.GetFileName(name);
				}
				
				// This may exceed windows length restrictions.
				// Combine will throw a PathTooLongException in that case.
				if ( _baseDirectory != null ) {
					name = Alphaleonis.Win32.Filesystem.Path.Combine(_baseDirectory, name);
				}	
			}
			else {
				name = string.Empty;
		...
		static WindowsNameTransform()
		{
			...
			invalidPathChars = Alphaleonis.Win32.Filesystem.Path.InvalidPathChars;
#else
			invalidPathChars = Alphaleonis.Win32.Filesystem.Path.GetInvalidPathChars();
#endif
