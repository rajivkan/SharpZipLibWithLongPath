...
namespace ICSharpCode.SharpZipLib.Zip 
{
	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public ZipFile(string name)
		{
            ...
            baseStream_ = Alphaleonis.Win32.Filesystem.File.Open(name, FileMode.Open, FileAccess.Read, FileShare.Read);
			isStreamOwner = true;
		...
		public static ZipFile Create(string fileName)
		{
			...
			FileStream fs = Alphaleonis.Win32.Filesystem.File.Create(fileName);
 ...
 */
		...
		void Reopen()
		{
            ...
            Reopen(Alphaleonis.Win32.Filesystem.File.Open(Name, FileMode.Open, FileAccess.Read, FileShare.Read));
		}
		...
		void RunUpdates()
		{
			...
			try {
				...
				if (!directUpdate && (workFile.Name != null)) {
					Alphaleonis.Win32.Filesystem.File.Delete(workFile.Name);
				}
				throw;
			}
	...
	public class StaticDiskDataSource : IStaticDataSource
	{
		...
		public Stream GetSource()
		{
            return Alphaleonis.Win32.Filesystem.File.Open(fileName_, FileMode.Open, FileAccess.Read, FileShare.Read);
		}
	...
	public class DynamicDiskDataSource : IDynamicDataSource
	{
		...
		public Stream GetSource(ZipEntry entry, string name)
		{
			...
			if ( name != null ) {
                result = Alphaleonis.Win32.Filesystem.File.Open(name, FileMode.Open, FileAccess.Read, FileShare.Read);
			}
	...
	public class DiskArchiveStorage : BaseArchiveStorage
	{
		...
		public override Stream GetTemporaryOutput()
		{
			if ( temporaryName_ != null ) {
                ...
                temporaryStream_ = Alphaleonis.Win32.Filesystem.File.Open(temporaryName_, FileMode.OpenOrCreate, FileAccess.Write, FileShare.None);
			}
			else {
				// Determine where to place files based on internal strategy.
				// Currently this is always done in system temp directory.
				temporaryName_ = Alphaleonis.Win32.Filesystem.Path.GetTempFileName();
                temporaryStream_ = Alphaleonis.Win32.Filesystem.File.Open(temporaryName_, FileMode.OpenOrCreate, FileAccess.Write, FileShare.None);
			}
		...
		public override Stream ConvertTemporaryToFinal()
		{
			...
			try	{
				...
				Alphaleonis.Win32.Filesystem.File.Move(fileName_, moveTempName);
				Alphaleonis.Win32.Filesystem.File.Move(temporaryName_, fileName_);
				newFileCreated = true;
				Alphaleonis.Win32.Filesystem.File.Delete(moveTempName);

                result = Alphaleonis.Win32.Filesystem.File.Open(fileName_, FileMode.Open, FileAccess.Read, FileShare.Read);
			}
			catch(Exception) {
				result  = null;
				...
				if ( !newFileCreated ) {
					Alphaleonis.Win32.Filesystem.File.Move(moveTempName, fileName_);
					Alphaleonis.Win32.Filesystem.File.Delete(temporaryName_);
				}
		...
		public override Stream MakeTemporaryCopy(Stream stream)
		{
			...
			Alphaleonis.Win32.Filesystem.File.Copy(fileName_, temporaryName_, true);
		...
		static string GetTempFileName(string original, bool makeTempFile)
		{
			...
			if ( original == null ) {
				result = Alphaleonis.Win32.Filesystem.Path.GetTempFileName();
			}
			else {
				int counter = 0;
				...
				while ( result == null ) {
					...
					if ( !Alphaleonis.Win32.Filesystem.File.Exists(newName) ) {
						if ( makeTempFile) {
							try	{
								// Try and create the file.
								using ( FileStream stream = Alphaleonis.Win32.Filesystem.File.Create(newName) ) {
								}
								result = newName;
							}
							catch {
